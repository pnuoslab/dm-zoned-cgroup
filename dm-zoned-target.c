...
struct dm_cgrp {
	...
	long long		budget;
	unsigned int		issued_bw;
	unsigned int		processed_bw;
	unsigned int		queued_bw;
...
static void dmz_refill_budget(struct work_struct *work)
{
	...
	/*
	cgrp_data->budget = (dmz->total_bw * weight / total_weight);
	if (dmz->high_weight == weight && budget_left)
		cgrp_data->budget -= (cgrp_data->budget / 10);
	if(dmz->high_weight == weight && !budget_left)
		cgrp_data->budget += (dmz->high_bw * weight / dmz->high_weight);
	*/

	/*
	printk("%s: added budget %lld high bw %lld, high weight %u, this weight %u",__func__,
			cgrp_data->budget,
			dmz->high_bw,
			dmz->high_weight,
			cgrp_data->weight);
	*/
	...
	if (dmz->donate > 0){
		//need change
		cgrp_data->budget += (dmz->donate * weight / total_weight);
		dmz->donate -= (dmz->donate * weight / total_weight);
	}
	*/
...
static int dmz_queue_cgrp_work(struct dmz_target *dmz, struct bio *bio)
{
	...
	if (cgw) {
		...
		if (cgw->cgrp_data == NULL){
			...
			cgw->cgrp_data->issued_bw = 0;
			cgw->cgrp_data->processed_bw = 0;
			cgw->cgrp_data->queued_bw = 0;
	...
	if (cgw->cgrp_data->weight){
		cgw->cgrp_data->issued_bw += bio_kb;
		if (cgw->cgrp_data->budget > 0){
			...
			cgw->cgrp_data->queued_bw += bio_kb;
			cgw->cgrp_data->last_added = jiffies;
