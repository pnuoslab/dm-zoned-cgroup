...
void dmz_mapping_cgroup(struct dmz_qnode *qn) { 
	printk("CSS ID %d's queue node is activated",qn->id);
	qn->dcg->created = true;
}

void dmz_check_bio(struct dmz_metadata *zmd, int css_id){
	struct dmz_qnode *check;

	list_for_each_entry(check, &zmd->umapped, node){
		if (check->id == css_id) {
			if (!check->dcg->created){
				dmz_mapping_cgroup(check);
			}
			goto end_search;
		}
	}
		     ...
		     const char *devname)
{
	...
	//added
	//INIT_LIST_HEAD(&zmd->qn->node);
	//zmd->cur_cg = NULL;
	//qn = kzalloc(sizeof(*qn), GFP_KERNEL);
	//zmd->head = qn;
	//INIT_LIST_HEAD(&zmd->head->node);
	//INIT_LIST_HEAD(&zmd->mapped);
	//INIT_LIST_HEAD(&zmd->umapped);
	 ...
	 */
	/*
	for (i = 0 ; i < DMZ_MAX_CGROUP ; i++){
		qn = kzalloc(sizeof(*qn), GFP_KERNEL);
		qn->id = i;
		list_add_tail(&qn->node, &zmd->umapped);
		//printk("struct no %d is created",qn->id);
		qn->dcg->created = false;
		//zmd->node_list[i] = qn;
	}
	*/
